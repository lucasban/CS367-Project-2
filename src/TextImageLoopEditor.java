...
public class TextImageLoopEditor extends ImageLoopEditor {
	...
	public static void textInterface(String[] args) {
		...
		boolean done = false;
		String[] validCommands = { "save", "load", "add", "insert", "find", "update", "edit", "jump", "display", "show",
				"test", "remove", "forward", "backward", "quit", "help" };
		...
		while (!done) {
			...
			if (command.length() > 0) {
				...
				// (maybe a prefix)
				List<String> possibleCommands = selectByPrefix(tokens[0], validCommands);
